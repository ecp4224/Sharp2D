...
namespace SomeGame
{
    public class Firefly : Light, ILogical
    {
        ...
        private float TargetIntensity = 3f;
        ...
        public void Update()
        {
            ...
            sprite.Alpha = Intensity/TargetIntensity;
        }
        ...
        private void Lighting()
        {
            ...
            if (litup && Intensity < TargetIntensity)
            {
                Intensity = ease(0, TargetIntensity, duration, Screen.TickCount - start);
            }
            else if (!litup && Intensity > 0f)
            {
                Intensity = ease(TargetIntensity, 0, duration, Screen.TickCount - start);
                if (Intensity == 0)
                {
        ...
        public Firefly()
            : base(0, 0, 0f, 10f, SKColors.Gold, LightType.DynamicPointLight)
        {
        ...
        public override void Load()
        {
            ...
            sprite = Sprite.FromImage("sprites/ball.png");
            sprite.TintColor = SKColors.Yellow;
            sprite.X = X;
            sprite.Y = Y;
            sprite.Scale = 0.125f;
            Attach(sprite);
